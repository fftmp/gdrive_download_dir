...
def get_filelist(dir_name, dir_id):
    log.debug('get files from folder with id = ' + dir_id)
    files = dict()
    next_page_token = ''
    ...
    while True:
        ...
        for item in res['items']:
            if item['mimeType'] == 'application/vnd.google-apps.folder':
                nested_files = get_filelist(item['title'], item['id'])
...
def download_file(file_id, filename):
    ...
    log.debug('Try to download ' + filename + ' from ' + file_id + ' path = ' + path)
...
def main():
    ...
    files = get_filelist(sys.argv[1], sys.argv[1])
